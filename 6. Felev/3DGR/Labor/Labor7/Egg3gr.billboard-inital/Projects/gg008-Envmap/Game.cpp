...
HRESULT Game::createResources() {
	...
	firstPersonCam = Egg::Cam::FirstPerson::create()
		->setView(float3(0, 0, 200), float3(0, 0, -1))
	...
	fireMaterial->setConstantBuffer("perFrame", perFrameConstantBuffer);
	...
	ID3D11BlendState* additiveBlender;
	device->CreateBlendState(&blendDesc, &additiveBlender);
	...
	fireMaterial->setDepthStencilState(depthState);
	...
	fireMaterial->setRasterizerState(rasterizerState);
